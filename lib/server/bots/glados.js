...
const getTurns = (botToken, getState, turnsPlayedByBot, successfullTurnsPlayedByBot, viewerBoard) => {
  ...
  while (lookForAvailableSpot) {
    ...
    const smartGuess = Math.floor(Math.random() * 30) < turnsPlayedByBot.length ? validPositions[Math.floor(Math.random() * validPositions.length)] : guessPool.length > 0 ? guessPool[Math.floor(Math.random() * guessPool.length)] : false;
