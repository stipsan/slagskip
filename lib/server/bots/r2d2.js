...
const getTurns = (botToken, getState, turnsPlayedByBot, successfullTurnsPlayedByBot) => {
  ...
  console.log('before cleanup', [].concat(_toConsumableArray(continuePreviousHits)));
  // remove already played turns
  turnsPlayedByBot.forEach(turnIndex => {
    continuePreviousHits.delete(turnIndex);
  });
  console.log('after cleanup', [].concat(_toConsumableArray(continuePreviousHits)));
  const guessPool = [].concat(_toConsumableArray(continuePreviousHits));
