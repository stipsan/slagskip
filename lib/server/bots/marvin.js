...
const getTurns = (botToken, getState, turnsPlayedByBot, successfullTurnsPlayedByBot, viewerBoard) => {
  ...
  const validPositions = viewerBoard.reduce((previousValidPositions, position, index) => {
    return position > 0 && turnsPlayedByBot.indexOf(index) === -1 ? [].concat(_toConsumableArray(previousValidPositions), [index]) : previousValidPositions;
  }, []);
  ...
  while (lookForAvailableSpot) {
    const guessPool = [].concat(continuePreviousHits);
    const smartGuess = Math.floor(Math.random() * 70) < turnsPlayedByBot.length ? validPositions[Math.floor(Math.random() * validPositions.length)] : guessPool.length > 0 ? guessPool[Math.floor(Math.random() * guessPool.length)] : false;
